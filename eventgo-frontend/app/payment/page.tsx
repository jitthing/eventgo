...
function PaymentForm({ eventId, seats, total, reservationId, userId, userEmail, onSuccess }: PaymentFormProps) {
	...
	useEffect(() => {
		...
		async function createPaymentIntent(): Promise<void> {
			try {
				...
				const response = await fetch(`${process.env.NEXT_PUBLIC_PAYMENTS_API_URL || "http://localhost:8004"}/create-payment-intent`, {
					method: "POST",
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
		...
		try {
			...
			if (paymentIntent?.status === "succeeded") {
				...
				const confirmResponse = await fetch(`${process.env.NEXT_PUBLIC_PAYMENTS_API_URL || "http://localhost:8004"}/confirm-booking`, {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
				...
				const processBookingResponse = await fetch(`${process.env.NEXT_PUBLIC_BOOKING_SERVICE_API_URL || "http://localhost:8007"}/bookings/process-booking`, {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
						eventId: eventId,
						...
						userEmail: userEmail ? userEmail : "user1@example.com",
						totalAmount: Math.round(parseFloat(total ?? "0")),
					}),
				});
	...
	return (
		<form onSubmit={handleSubmit}>
			...
			<button
				...
				className={`w-full text-center bg-blue-600 text-white font-medium py-3 px-8 rounded-md hover:bg-blue-700 transition-colors ${loading || !stripe || !clientSecret ? "opacity-50 cursor-not-allowed" : ""}`}
			>
