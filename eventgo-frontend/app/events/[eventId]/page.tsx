...
export default function EventPage() {
	...
	const toggleSeat = (seatId: number) => {
		setSelectedSeats((prev) => {
			const newSelectedSeats = prev.includes(seatId) ? prev.filter((s) => s !== seatId) : [...prev, seatId];
		...
		<div className="relative min-h-screen bg-white py-12 px-4 sm:px-6 lg:px-8">
			...
			<div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10">
				...
				<div className="p-6 bg-white shadow-lg rounded-lg">
					...
					<SeatSelection eventId={event.event_id} selectedSeats={selectedSeats} toggleSeat={toggleSeat} availableSeats={availableSeats} />

					{/* Display total price */}
					{selectedSeats.length > 0 && (
						<div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
							<p className="text-gray-900 font-medium">Total Price: ${totalPrice.toFixed(2)}</p>
							<p className="text-sm text-gray-600">For {selectedSeats.length} seat(s)</p>
						</div>
					)}
			...
			{isGroupBookingOpen && <GroupBookingModal selectedSeats={selectedSeats} eventId={eventId} onClose={closeGroupBooking} totalPrice={totalPrice} availableSeats={availableSeats} />}
		</div>
	);
}

function GroupBookingModal({ selectedSeats, eventId, onClose, totalPrice, availableSeats }: { selectedSeats: number[]; eventId: number; onClose: () => void; totalPrice: number; availableSeats: any[] }) {
	const router = useRouter();
	...
	return (
		<div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
			<div className="bg-white p-8 rounded-lg shadow-lg w-[500px] max-w-full">
				<h2 className="text-2xl font-bold text-black text-center">Split Payment Booking</h2>
				...
				<div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md text-center">
					<p className="text-black font-medium">Total Price: ${totalPrice.toFixed(2)}</p>
					<p className="text-sm text-gray-600">Price per person: ${(totalPrice / selectedSeats.length).toFixed(2)}</p>
				</div>
				...
				<div className="mt-6 flex justify-between">
					...
					<button onClick={proceedToCheckout} className={`py-2 px-4 rounded text-white ${isAllSeatsAssigned ? "bg-green-600 hover:bg-green-700" : "bg-green-400 cursor-not-allowed opacity-90"}`} disabled={!isAllSeatsAssigned}>
						Proceed to Checkout
					</button>
				</div>
			</div>
